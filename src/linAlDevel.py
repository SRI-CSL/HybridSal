...
# Todo:
# 1. extendToFull: extend in space orthogonal to A*basis
# 2. changeOfBasis: assumed orthonormal basis, not true anymore; change
# 3. inverseA: Need code for inverting A? Or better to solve n equations?
# 4. replace dict returned by neigenvalues by list of tuples
#   tuples (lamb, multiplicity, coefficients)
# 5. Then use coefficients to search for generalized eigenvectors
# 6. If multiplicity >= 4, then we may not be able to handle presently...
...
def extendToFull(basis, A, n):
    """Extend the partial orthonormal basis to a full basis
       by adding vectors in the ORTHO-space of A^T*basis"""
    j = len(basis)
    i = 0
    while (j < n and i < n):
...
def charPolPart(A, vec, degree):
    """Find a factor of the characteristic polynomial of A."""
    return None
...
def neigenvalues(A):
    ...
    coeffs = charPolPart(A, vec, done)
    newbasis = extendToFull(subspace, A, n)
