...
Sankaranarayanan and Tiwari~\cite{ST11:CAV}.
...
using infinite bounded model checking, 
...
\footnote{We have not tried abstract interpretation yet.}.
The performance of HybridSal matches the performance reported 
in our earlier paper~\cite{ST11:CAV} on the navigation
benchmarks (which are included with the HybridSal distribution).
In~\cite{ST11:CAV} we had used many
...
(such as inductive invariants or Lyapunov function)~\cite{sostools,ST11:ISSAC},
or
(c) tools that compute an abstraction and then analyze
the abstraction~\cite{HybridSAL,AlurDangIvancic03:TACAS,Clarke03:TACAS}.
