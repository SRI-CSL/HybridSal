...
\begin{itemize}
\item Switch to {\cem{NE}} mode when moving left and $-1.5 \leq x \leq -1$
\item Switch to {\cem{NW}} mode when moving right and $1 \leq x \leq 1.5$
\end{itemize}

{\crm{A possible simulation}} trajectory:
...
{\cem{Does the robot ever hit the wall}} -- 
for all initial states and switchings?
%irrespective of where it starts, and how it switches ?
...
% -------------------------------------------------------
\begin{slide}
\heading{WHY Relational Abstraction}

{\cem{Concept}}: Analyze hybrid systems by first replacing 
ODEs by their relational abstraction

{\cem{Why is this a good idea?}}
\begin{itemize}
\item 
 {\crm{separation of concerns}}
 \begin{itemize}
 ...
 \end{itemize}
\item
 {\crm{accuracy improves}} as we get closer to decidable classes
 \begin{itemize}
 \item  relationalization is {\cem{lossless}} for timed automata, LHAs
 \item  almost lossless for other decidable classes of CDSs
 \end{itemize}
\item 
 {\cem{good quality abstractions automatically computed for linear ODEs}}
\item
 {\cem{generalizes}} to timed relational abstraction etc.
\end{itemize}
